...
const statusOptions: Array<{ label: string; value: AttendanceRecord["status"] | "" }> = [
  { label: "Todos los estados", value: "" },
  { label: "Presentes", value: "PRESENT" },
  { label: "Tardanzas", value: "LATE" },
  { label: "Permisos", value: "ON_LEAVE" },
  { label: "Ausentes", value: "ABSENT" },
];

export function AttendanceRecordFiltersForm({
  action,
  companies,
  filters,
  hiddenFields,
  ...
  return (
    ...
    >
      {Object.entries(hiddenFields).map(([name, value]) => (
        <input autoComplete="off" key={name} name={name} type="hidden" value={value} />
      ))}
      ...
      <label className="flex h-11 min-w-0 items-center gap-2 rounded-xl border border-[#e1e5eb] bg-white px-3 text-sm text-[#475467]">
        ...
        <input autoComplete="off"
          ...
          name="buscar"
          placeholder="Buscar trabajador, cargo o empresa"
        />
      </label>
