...
import { createDocumentFolder } from "./api";
...
export function DocumentFolderForm() {
  ...
  return (
    <>
      <button
        className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-[#d8dee8] bg-white px-4 text-sm font-semibold text-[#475467] transition hover:border-[#4f46e5] hover:bg-[#eef2ff] hover:text-[#4f46e5]"
              ...
              <div className="animate-rise max-h-[calc(100dvh-48px)] w-full max-w-2xl overflow-hidden rounded-2xl border border-[#e1e5eb] bg-white shadow-[0_28px_90px_rgba(16,24,40,0.24)]">
                <div className="flex items-start justify-between gap-4 border-b border-[#e1e5eb] px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#667085]">
                      Define la categoria, visibilidad y si pedira firma.
                    </p>
                  </div>
                  <button
                    aria-label="Cerrar"
                ...
                <form className="space-y-4 px-5 py-5" onSubmit={handleSubmit}>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Nombre">
                      <input className={inputClassName} name="name" placeholder="Ej. Boletas" required />
                    </Field>
                    ...
                    <Field label="Retencion">
                      <input className={inputClassName} min="0" name="retentionYears" placeholder="Años, opcional" type="number" />
                    </Field>
                    <Field label="Descripcion">
                      <input className={inputClassName} name="description" placeholder="Uso de esta carpeta" />
                    </Field>
                  </div>

                  <div className="grid gap-3 sm:grid-cols-3">
                    <Toggle defaultChecked label="Visible en portal" name="visibleToEmployee" />
                    <Toggle label="Requiere firma" name="requiresSignature" />
                    <Toggle defaultChecked label="Permite varios" name="allowMultiple" />
                  </div>

                  <div className="min-h-9">
                    {state === "loading" ? <ActionFeedback message="Creando carpeta..." tone="loading" /> : null}
                    {state === "error" ? <ActionFeedback message={message} tone="error" /> : null}
                  </div>

                  <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    <button className="h-10 rounded-xl border border-[#d8dee8] bg-white px-4 text-sm font-semibold text-[#475467]" onClick={closeModal} type="button">
                      Cancelar
                    </button>
                    <button className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#4f46e5] px-4 text-sm font-semibold text-white" disabled={state === "loading"} type="submit">
                      {state === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : <FolderPlus className="h-4 w-4" />}
                      Crear carpeta
                    </button>
                  </div>
                </form>
