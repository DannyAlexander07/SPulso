import Link from "next/link";
import {
  ArrowLeft,
  BellRing,
  Building2,
  CalendarCheck,
  Clock3,
  FileText,
  History,
  MapPin,
  ShieldCheck,
  UsersRound,
} from "lucide-react";
import { AppSidebar } from "@/components/layout/app-sidebar";
import { AppTopbar } from "@/components/layout/app-topbar";
import type { AuthUser } from "@/features/auth/types";
import type { CompanyProfile } from "./types";

export function CompanyProfileView({
  currentUser,
  profile,
}: {
  currentUser: AuthUser | null;
  profile: CompanyProfile;
}) {
  const company = profile.company;
  const openAlerts = profile.notifications.filter((notification) => notification.status === "UNREAD");
  const pendingDocuments = profile.documents.filter((document) =>
    ["EXPIRED", "PENDING_SIGNATURE"].includes(document.status),
  );
  const pendingRequests = profile.requests.filter((request) => request.status === "PENDING");

  return (
    <main className="min-h-screen bg-[#f4f6f8] text-[#1f242d]">
      <div className="grid min-h-screen lg:grid-cols-[216px_minmax(0,1fr)]">
        <AppSidebar activePath="/empresas" currentUser={currentUser} />

        <section className="min-w-0">
          <AppTopbar currentUser={currentUser} eyebrow="Empresas" title="Perfil 360 de empresa" />

          <div className="w-full px-4 py-4 pb-24 sm:px-5 lg:px-6 lg:pb-4">
            <Link
              className="mb-4 inline-flex h-10 items-center gap-2 rounded-xl border border-[#e1e5eb] bg-white px-3 text-sm font-semibold text-[#475467] shadow-sm transition hover:bg-[#f8fafc]"
              href="/empresas"
            >
              <ArrowLeft className="h-4 w-4" />
              Volver a empresas
            </Link>

            <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
              <div className="rounded-2xl border border-[#e1e5eb] bg-white p-5 shadow-sm">
                <div className="flex min-w-0 gap-4">
                  <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#eef2ff] text-xl font-bold text-[#4f46e5]">
                    {company.name.slice(0, 2).toUpperCase()}
                  </span>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#667085]">
                      Operacion multiempresa
                    </p>
                    <h1 className="mt-1 text-3xl font-semibold tracking-normal">{company.name}</h1>
                    <p className="mt-2 text-sm text-[#667085]">
                      {company.slug} · RUC {company.ruc ?? "pendiente"}
                    </p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Badge label={company.status === "ACTIVE" ? "Activa" : "Inactiva"} tone={company.status === "ACTIVE" ? "emerald" : "slate"} />
                      <Badge label={`Entrada ${company.workStartTime}`} tone="blue" />
                      <Badge label={`Tolerancia ${company.lateToleranceMinutes} min`} tone="amber" />
                    </div>
                  </div>
                </div>

                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <InfoTile icon={UsersRound} label="Trabajadores" value={String(profile.employeeSummary.total)} />
                  <InfoTile icon={ShieldCheck} label="Activos" value={String(profile.employeeSummary.active)} />
                  <InfoTile icon={CalendarCheck} label="Asistencia hoy" value={String(profile.attendanceSummary.total)} />
                  <InfoTile icon={MapPin} label="Tardanzas hoy" value={String(profile.attendanceSummary.late)} />
                </div>
              </div>

              <div className="rounded-2xl border border-[#e1e5eb] bg-white p-5 text-[#1f242d] shadow-sm">
                <p className="text-sm font-semibold">Pulso operativo</p>
                <p className="mt-1 text-xs text-[#667085]">Pendientes que requieren atencion.</p>
                <div className="mt-5 space-y-3">
                  <DarkMetric label="Alertas abiertas" value={openAlerts.length} tone={openAlerts.length > 0 ? "red" : "emerald"} />
                  <DarkMetric label="Documentos por atender" value={pendingDocuments.length} tone={pendingDocuments.length > 0 ? "amber" : "emerald"} />
                  <DarkMetric label="Solicitudes pendientes" value={pendingRequests.length} tone={pendingRequests.length > 0 ? "amber" : "emerald"} />
                  <DarkMetric label="Ausencias hoy" value={profile.attendanceSummary.absent} tone={profile.attendanceSummary.absent > 0 ? "red" : "emerald"} />
                </div>
              </div>
            </section>

            <section className="mt-4 grid gap-4 lg:grid-cols-4">
              <MetricCard label="Presentes" value={profile.attendanceSummary.present} tone="emerald" />
              <MetricCard label="Tardanzas" value={profile.attendanceSummary.late} tone="amber" />
              <MetricCard label="Permisos" value={profile.attendanceSummary.onLeave} tone="blue" />
              <MetricCard label="Ausencias" value={profile.attendanceSummary.absent} tone="red" />
            </section>

            <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
              <EmployeesPanel employees={profile.employees} />
              <AlertsPanel notifications={profile.notifications} />
            </section>

            <section className="mt-4 grid gap-4 xl:grid-cols-3">
              <AttendancePanel records={profile.attendance} />
              <DocumentsPanel documents={profile.documents} />
              <RequestsPanel requests={profile.requests} />
            </section>

            <section className="mt-4">
              <AuditPanel logs={profile.auditLogs} />
            </section>
          </div>
        </section>
      </div>
    </main>
  );
}

function EmployeesPanel({ employees }: { employees: CompanyProfile["employees"] }) {
  return (
    <Panel icon={UsersRound} title="Equipo de la empresa" eyebrow="Personas">
      <div className="grid gap-2 md:grid-cols-2">
        {employees.length > 0 ? (
          employees.map((employee) => (
            <Link
              className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3 transition hover:border-[#818cf8] hover:bg-white"
              href={`/trabajadores/${employee.id}`}
              key={employee.id}
            >
              <p className="whitespace-normal break-words text-sm font-semibold leading-5">{employee.firstName} {employee.lastName}</p>
              <p className="mt-1 whitespace-normal break-words text-xs leading-4 text-[#667085]">{employee.jobTitle ?? "Sin cargo"} · {employee.area ?? "Sin area"}</p>
            </Link>
          ))
        ) : (
          <div className="md:col-span-2"><EmptyState text="No hay trabajadores registrados." /></div>
        )}
      </div>
    </Panel>
  );
}

function AlertsPanel({ notifications }: { notifications: CompanyProfile["notifications"] }) {
  return (
    <Panel icon={BellRing} title="Alertas de empresa" eyebrow="Automatizaciones">
      <div className="space-y-2">
        {notifications.length > 0 ? (
          notifications.map((notification) => (
            <div className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3" key={notification.id}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="whitespace-normal break-words text-sm font-semibold leading-5">{notification.title}</p>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-[#667085]">{notification.message}</p>
                </div>
                <Badge label={priorityLabel(notification.priority)} tone={priorityTone(notification.priority)} />
              </div>
            </div>
          ))
        ) : (
          <EmptyState text="No hay alertas de esta empresa." />
        )}
      </div>
    </Panel>
  );
}

function AttendancePanel({ records }: { records: CompanyProfile["attendance"] }) {
  return (
    <Panel icon={CalendarCheck} title="Asistencia hoy" eyebrow="Control diario">
      <div className="space-y-2">
        {records.length > 0 ? (
          records.map((record) => (
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3" key={record.id}>
              <div className="min-w-0">
                <p className="whitespace-normal break-words text-sm font-semibold leading-5">{record.employee.firstName} {record.employee.lastName}</p>
                <p className="mt-0.5 text-xs text-[#667085]">Entrada {formatTime(record.checkIn)}</p>
              </div>
              <Badge label={attendanceLabel(record.status)} tone={attendanceTone(record.status)} />
            </div>
          ))
        ) : (
          <EmptyState text="No hay asistencia registrada hoy." />
        )}
      </div>
    </Panel>
  );
}

function DocumentsPanel({ documents }: { documents: CompanyProfile["documents"] }) {
  return (
    <Panel icon={FileText} title="Documentos" eyebrow="Gestion documental">
      <div className="space-y-2">
        {documents.length > 0 ? (
          documents.map((document) => (
            <div className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3" key={document.id}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="whitespace-normal break-words text-sm font-semibold leading-5">{document.title}</p>
                  <p className="mt-0.5 whitespace-normal break-words text-xs leading-4 text-[#667085]">{document.employee.firstName} {document.employee.lastName}</p>
                </div>
                <Badge label={documentStatusLabel(document.status)} tone={documentStatusTone(document.status)} />
              </div>
            </div>
          ))
        ) : (
          <EmptyState text="No hay documentos en esta empresa." />
        )}
      </div>
    </Panel>
  );
}

function RequestsPanel({ requests }: { requests: CompanyProfile["requests"] }) {
  return (
    <Panel icon={Clock3} title="Solicitudes" eyebrow="Aprobaciones">
      <div className="space-y-2">
        {requests.length > 0 ? (
          requests.map((request) => (
            <div className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3" key={request.id}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="whitespace-normal break-words text-sm font-semibold leading-5">{request.title}</p>
                  <p className="mt-0.5 whitespace-normal break-words text-xs leading-4 text-[#667085]">{request.employee.firstName} {request.employee.lastName}</p>
                </div>
                <Badge label={requestStatusLabel(request.status)} tone={requestStatusTone(request.status)} />
              </div>
            </div>
          ))
        ) : (
          <EmptyState text="No hay solicitudes en esta empresa." />
        )}
      </div>
    </Panel>
  );
}

function AuditPanel({ logs }: { logs: CompanyProfile["auditLogs"] }) {
  return (
    <Panel icon={History} title="Historial de empresa" eyebrow="Auditoria">
      <div className="grid gap-2 md:grid-cols-2">
        {logs.length > 0 ? (
          logs.map((log) => (
            <div className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3" key={log.id}>
              <p className="text-sm font-semibold">{humanAction(log.action)}</p>
              <p className="mt-1 text-xs leading-5 text-[#667085]">{log.summary}</p>
              <p className="mt-2 text-xs font-medium text-[#667085]">{log.actorLabel} · {formatDate(log.createdAt)}</p>
            </div>
          ))
        ) : (
          <div className="md:col-span-2"><EmptyState text="No hay auditoria para esta empresa." /></div>
        )}
      </div>
    </Panel>
  );
}

function Panel({ children, eyebrow, icon: Icon, title }: { children: React.ReactNode; eyebrow: string; icon: React.ElementType; title: string }) {
  return (
    <section className="rounded-2xl border border-[#e1e5eb] bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eef2ff] text-[#4f46e5]">
          <Icon className="h-5 w-5" />
        </span>
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#667085]">{eyebrow}</p>
          <h2 className="mt-1 text-xl font-semibold">{title}</h2>
        </div>
      </div>
      {children}
    </section>
  );
}

function InfoTile({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-[#e1e5eb] bg-[#fbfcfd] p-3">
      <Icon className="h-4 w-4 text-[#4f46e5]" />
      <p className="mt-3 text-xs font-semibold text-[#667085]">{label}</p>
      <p className="mt-1 whitespace-normal break-words text-sm font-semibold leading-5">{value}</p>
    </div>
  );
}

function MetricCard({ label, tone, value }: { label: string; tone: "amber" | "blue" | "emerald" | "red"; value: number }) {
  const tones = {
    amber: "bg-[#fff7df] text-[#b86b00]",
    blue: "bg-[#eef2ff] text-[#4f46e5]",
    emerald: "bg-[#e0f2fe] text-[#0284c7]",
    red: "bg-[#fee4e2] text-[#b42318]",
  };

  return (
    <article className="rounded-2xl border border-[#e1e5eb] bg-white p-4 shadow-sm">
      <p className="text-sm font-semibold text-[#667085]">{label}</p>
      <p className={`mt-3 inline-flex rounded-xl px-3 py-1.5 text-2xl font-bold ${tones[tone]}`}>{value}</p>
    </article>
  );
}

function DarkMetric({ label, tone, value }: { label: string; tone: "amber" | "emerald" | "red"; value: number }) {
  const tones = { amber: "text-[#fbbf24]", emerald: "text-[#38bdf8]", red: "text-[#f87171]" };
  return (
    <div className="flex items-center justify-between rounded-xl border border-[#e1e5eb] bg-[#fbfcfd] px-3 py-3">
      <span className="text-sm text-[#667085]">{label}</span>
      <span className={`text-lg font-bold ${tones[tone]}`}>{value}</span>
    </div>
  );
}

function Badge({ label, tone }: { label: string; tone: "amber" | "blue" | "emerald" | "red" | "slate" }) {
  const tones = {
    amber: "bg-[#fff7df] text-[#b86b00]",
    blue: "bg-[#eef2ff] text-[#4f46e5]",
    emerald: "bg-[#e0f2fe] text-[#0284c7]",
    red: "bg-[#fee4e2] text-[#b42318]",
    slate: "bg-[#f2f4f7] text-[#667085]",
  };
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${tones[tone]}`}>{label}</span>;
}

function EmptyState({ text }: { text: string }) {
  return <p className="rounded-2xl border border-dashed border-[#d8dee8] bg-[#fbfcfd] p-4 text-sm text-[#667085]">{text}</p>;
}

function formatDate(value: string) {
  return new Intl.DateTimeFormat("es-PE", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(value));
}

function formatTime(value: string | null) {
  if (!value) return "Sin marca";
  return new Intl.DateTimeFormat("es-PE", { hour: "2-digit", minute: "2-digit" }).format(new Date(value));
}

function attendanceLabel(status: CompanyProfile["attendance"][number]["status"]) {
  return { ABSENT: "Ausente", LATE: "Tardanza", ON_LEAVE: "Permiso", PRESENT: "Presente" }[status];
}

function attendanceTone(status: CompanyProfile["attendance"][number]["status"]) {
  return ({ ABSENT: "red", LATE: "amber", ON_LEAVE: "blue", PRESENT: "emerald" } as const)[status];
}

function documentStatusLabel(status: CompanyProfile["documents"][number]["status"]) {
  return { DRAFT: "Borrador", EXPIRED: "Vencido", PENDING_SIGNATURE: "Por firmar", SIGNED: "Firmado" }[status];
}

function documentStatusTone(status: CompanyProfile["documents"][number]["status"]) {
  return ({ DRAFT: "slate", EXPIRED: "red", PENDING_SIGNATURE: "amber", SIGNED: "emerald" } as const)[status];
}

function requestStatusLabel(status: CompanyProfile["requests"][number]["status"]) {
  return { APPROVED: "Aprobada", CANCELLED: "Cancelada", PENDING: "Pendiente", REJECTED: "Rechazada" }[status];
}

function requestStatusTone(status: CompanyProfile["requests"][number]["status"]) {
  return ({ APPROVED: "emerald", CANCELLED: "slate", PENDING: "amber", REJECTED: "red" } as const)[status];
}

function priorityLabel(priority: CompanyProfile["notifications"][number]["priority"]) {
  return { CRITICAL: "Critica", INFO: "Info", WARNING: "Atender" }[priority];
}

function priorityTone(priority: CompanyProfile["notifications"][number]["priority"]) {
  return ({ CRITICAL: "red", INFO: "blue", WARNING: "amber" } as const)[priority];
}

function humanAction(action: string) {
  const labels: Record<string, string> = {
    "company.attendance_rules.updated": "Reglas de asistencia actualizadas",
    "company.created": "Empresa creada",
    "company.updated": "Empresa actualizada",
    "document.created": "Documento creado",
    "document.updated": "Documento actualizado",
    "employee.created": "Trabajador creado",
    "employee.updated": "Trabajador actualizado",
  };

  return labels[action] ?? "Cambio registrado";
}
