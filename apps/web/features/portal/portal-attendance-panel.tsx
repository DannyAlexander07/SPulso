...
import { requestBrowserLocation, type BrowserLocation } from "@/features/worker-attendance/location-utils";
...
export function PortalAttendancePanel({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("success");
      setMessage(selectedAction === "CHECK_IN" ? "Entrada registrada correctamente." : "Salida registrada correctamente.");
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-[26px] border border-[#dfe5ee] bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#667085]">Jornada de hoy</p>
          <Link
        ...
        <div className="mt-3 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-semibold tracking-normal text-[#171b23]">{currentTime}</h1>
            <p className="mt-2 text-sm text-[#667085]">Marcacion con GPS obligatorio y PIN personal.</p>
          </div>
        ...
        <LocationControl locationMessage={locationMessage} locationState={locationState} onActivate={activateLocation} />
      </section>

      <section className="rounded-[26px] border border-[#dfe5ee] bg-white p-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#667085]">Registrar marca</p>
        ...
        <form autoComplete="off" className="mt-5 space-y-4" onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-2 rounded-2xl bg-[#f3f5f8] p-1.5">
            <ActionSwitch active={selectedAction === "CHECK_IN"} icon={LogIn} label="Entrada" onClick={() => setSelectedAction("CHECK_IN")} />
            <ActionSwitch active={selectedAction === "CHECK_OUT"} icon={LogOut} label="Salida" onClick={() => setSelectedAction("CHECK_OUT")} />
          </div>
        ...
        <Link className="inline-flex h-10 w-full items-center justify-center rounded-xl bg-[#f8fafc] px-4 text-sm font-semibold text-[#475467] transition hover:bg-[#eef2f7]" href="/cambiar-pin">
...
function ActionSwitch({ active, icon: Icon, label, onClick }: { active: boolean; icon: React.ElementType; label: string; onClick: () => void }) {
  return (
    <button
      className={`flex h-11 items-center justify-center gap-2 rounded-xl text-sm font-semibold transition ${
        active ? "bg-white text-[#4f46e5] shadow-sm" : "text-[#667085] hover:bg-white/70 hover:text-[#20242c]"
      }`}
      onClick={onClick}
      type="button"
    >
